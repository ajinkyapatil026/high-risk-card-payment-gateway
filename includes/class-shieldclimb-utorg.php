...
add_action('plugins_loaded', 'shieldclimbgateway_utorg_gateway');

function shieldclimbgateway_utorg_gateway() {
...
function shieldclimbgateway_utorgpro_permission_callback($request) {
    ...
    if ($order->get_meta('shieldclimb_utorgpro_nonce', true) !== $nonce) {
        return new WP_Error(
            'rest_forbidden',
...
function shieldclimbgateway_utorgpro_change_order_status_rest_endpoint() {
    register_rest_route(
        ...
        '/shieldclimbgateway-utorgpro/',
        array(
            'methods'             => 'GET',
            'callback'            => 'shieldclimbgateway_utorgpro_change_order_status_callback',
            'permission_callback' => 'shieldclimbgateway_utorgpro_permission_callback',
        )
    );
}
...
function shieldclimbgateway_utorgpro_change_order_status_callback($request) {
    ...
    $shieldclimbgateway_utorgpropaid_value_coin = sanitize_text_field($request->get_param('value_coin'));
    $shieldclimbgateway_utorgprofloatpaid_value_coin = (float)$shieldclimbgateway_utorgpropaid_value_coin;

    // Check order status and payment method
    if ($order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'shieldclimb-utorg' === $order->get_payment_method()) {
        $shieldclimbgateway_utorgproexpected_amount = (float)$order->get_meta('shieldclimb_utorgpro_expected_amount', true);
        $shieldclimbgateway_utorgprothreshold = 0.60 * $shieldclimbgateway_utorgproexpected_amount;

        if ($shieldclimbgateway_utorgprofloatpaid_value_coin < $shieldclimbgateway_utorgprothreshold) {
            ...
            $order->add_order_note(sprintf(__('Order marked as failed: Payment received is less than 60%% of the order total. Customer may have changed the payment values on the checkout page. TXID: %1$s', 'shieldclimb-high-risk-card-payment-gateway'), $shieldclimbgateway_utorgpropaid_txid_out));
