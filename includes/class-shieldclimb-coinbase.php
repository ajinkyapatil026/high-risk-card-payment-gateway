...
add_action('plugins_loaded', 'init_shieldclimbgateway_coinbase_gateway');

function init_shieldclimbgateway_coinbase_gateway() {
...
function shieldclimbgateway_coinbasecom_change_order_status_rest_endpoint() {
    ...
    register_rest_route( 'shieldclimbgateway/v1', '/shieldclimbgateway-coinbasecom/', array(
        'methods'  => 'GET',
...
function shieldclimbgateway_coinbasecom_change_order_status_callback( $request ) {
	...
	$shieldclimbgateway_coinbasecomgetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	...
	// Verify nonce
    if ( empty( $shieldclimbgateway_coinbasecomgetnonce ) || $order->get_meta('shieldclimb_coinbasecom_nonce', true) !== $shieldclimbgateway_coinbasecomgetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-high-risk-card-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-coinbase'
    if ( $order && $order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'shieldclimb-coinbase' === $order->get_payment_method() ) {
