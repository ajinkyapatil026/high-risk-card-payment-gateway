...
add_action('plugins_loaded', 'init_shieldclimbgateway_simplex_gateway');

function init_shieldclimbgateway_simplex_gateway() {
...
function shieldclimbgateway_simplexcom_change_order_status_rest_endpoint() {
    ...
    register_rest_route( 'shieldclimbgateway/v1', '/shieldclimbgateway-simplexcom/', array(
        'methods'  => 'GET',
...
function shieldclimbgateway_simplexcom_change_order_status_callback( $request ) {
	...
	$shieldclimbgateway_simplexcomgetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	...
	// Verify nonce
    if ( empty( $shieldclimbgateway_simplexcomgetnonce ) || $order->get_meta('shieldclimb_simplexcom_nonce', true) !== $shieldclimbgateway_simplexcomgetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-high-risk-card-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-simplex'
    if ( $order && $order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'shieldclimb-simplex' === $order->get_payment_method() ) {
