...
add_action('plugins_loaded', 'shieldclimbgateway_bitnovocom_gateway');

function shieldclimbgateway_bitnovocom_gateway() {
...
function shieldclimbgateway_bitnovocom_permission_callback($request) {
    ...
    if ($order->get_meta('shieldclimb_bitnovocom_nonce', true) !== $nonce) {
        return new WP_Error(
            'rest_forbidden',
...
function shieldclimbgateway_bitnovocom_change_order_status_rest_endpoint() {
    register_rest_route(
        ...
        '/shieldclimbgateway-bitnovocom/',
        array(
            'methods'             => 'GET',
            'callback'            => 'shieldclimbgateway_bitnovocom_change_order_status_callback',
            'permission_callback' => 'shieldclimbgateway_bitnovocom_permission_callback',
        )
    );
}
...
function shieldclimbgateway_bitnovocom_change_order_status_callback($request) {
    ...
    if ($order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'shieldclimb-bitnovo' === $order->get_payment_method()) {
