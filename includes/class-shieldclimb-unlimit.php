...
add_action('plugins_loaded', 'shieldclimbgateway_unlimit_gateway');

function shieldclimbgateway_unlimit_gateway() {
...
function shieldclimbgateway_gateficom_permission_callback($request) {
    ...
    if ($order->get_meta('shieldclimb_gateficom_nonce', true) !== $nonce) {
        return new WP_Error(
            'rest_forbidden',
...
function shieldclimbgateway_gateficom_change_order_status_rest_endpoint() {
    register_rest_route(
        ...
        '/shieldclimbgateway-gateficom/',
        array(
            'methods'             => 'GET',
            'callback'            => 'shieldclimbgateway_gateficom_change_order_status_callback',
            'permission_callback' => 'shieldclimbgateway_gateficom_permission_callback',
        )
    );
}
...
function shieldclimbgateway_gateficom_change_order_status_callback($request) {
    ...
    $shieldclimbgateway_gateficompaid_value_coin = sanitize_text_field($request->get_param('value_coin'));
    $shieldclimbgateway_gateficomfloatpaid_value_coin = (float)$shieldclimbgateway_gateficompaid_value_coin;

    // Check order status and payment method
    if ($order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'shieldclimb-unlimit' === $order->get_payment_method()) {
        $shieldclimbgateway_gateficomexpected_amount = (float)$order->get_meta('shieldclimb_gateficom_expected_amount', true);
        $shieldclimbgateway_gateficomthreshold = 0.60 * $shieldclimbgateway_gateficomexpected_amount;

        if ($shieldclimbgateway_gateficomfloatpaid_value_coin < $shieldclimbgateway_gateficomthreshold) {
            ...
            $order->add_order_note(sprintf(__('Order marked as failed: Payment received is less than 60%% of the order total. Customer may have changed the payment values on the checkout page. TXID: %1$s', 'shieldclimb-high-risk-card-payment-gateway'), $shieldclimbgateway_gateficompaid_txid_out));
