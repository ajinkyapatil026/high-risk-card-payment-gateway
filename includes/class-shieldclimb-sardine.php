...
add_action('plugins_loaded', 'init_shieldclimbgateway_sardine_gateway');

function init_shieldclimbgateway_sardine_gateway() {
...
function shieldclimbgateway_sardineai_change_order_status_rest_endpoint() {
    ...
    register_rest_route( 'shieldclimbgateway/v1', '/shieldclimbgateway-sardineai/', array(
        'methods'  => 'GET',
...
function shieldclimbgateway_sardineai_change_order_status_callback( $request ) {
	...
	$shieldclimbgateway_sardineaigetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	...
	// Verify nonce
    if ( empty( $shieldclimbgateway_sardineaigetnonce ) || $order->get_meta('shieldclimb_sardineai_nonce', true) !== $shieldclimbgateway_sardineaigetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'shieldclimb-high-risk-card-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'shieldclimb-sardine'
    if ( $order && $order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'shieldclimb-sardine' === $order->get_payment_method() ) {
